    ...
    try {
        ...
        if (req.body != null) {
            ...
            getbinarytoken(token).then(function (binarytoken) {
                ...
                const { sabercommand } = req.body;

                addtoPall(jsonObject["binarytoken"], sabercommand).then(function (statusval) {
                    res.send(statusval);
...
//To Generate the Sabre session for the Sabre Traction
function addtoPall(binarytoken, sabercommand) {
    try {
        return new Promise(function (resolve, reject) {

            const add = require("../modules/sabercommands.js")
            add(binarytoken, sabercommand, (err, result) => {
                if (err) { // Best practice to handle your errors
                    console.log(err)
                } else { // Implement the logic, what you want to do once you recieve the response back 
                    console.log(result)
