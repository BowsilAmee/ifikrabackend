...
module.exports = (binarytoken, hostcommand, callback) => {
    // let sum = a + b
    let error = null
    ...
    const soapHeader = {
        ...
        'soapAction': 'https://sws-crt.cert.havail.sabre.com#SabreCommandLLSRQ',
    };
    //TODO: Move URL for ENV
    var filePath = "./wsdl/SaberCommand.xml";
    const xml = fs.readFileSync(filePath, 'utf-8');

    var str = xml.toString();
    var mapObj = {
        binaryTokenval: binarytoken,
        HostCommandval: hostcommand

    };

    str = str.replace(/binaryTokenval|HostCommandval/gi, function (matched) {
    ...
    try {
        return new Promise(function (resolve, reject) {
            ...
            if (obj != null) {
                ...
                if (obj["soap-env:Envelope"]["soap-env:Body"] != null) {
                    resolve(obj["soap-env:Envelope"]["soap-env:Body"]);
                }
                else {
                    resolve("Unable to execute sabre command.");
                }
            }
            else {
                reject(error);
